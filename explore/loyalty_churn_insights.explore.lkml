...
explore: loyalty_with_churn{

from: convoiq_loyalty_insights_call_level_table {
  view_label: "Loyalty Insights Details"
  label: "Loyalty-Churn Insights Report"
  fields: [ALL_FIELDS*,
  ...
  join: icm_summary_fact {
    ...
    sql_on: ${loyalty_with_churn.acss_call_id} = ${icm_summary_fact.acss_call_id};;
  ...
  join: churn_sum_fact {
    ...
    sql_on: ${icm_summary_fact.cust_id}=${churn_sum_fact.cust_id}  and ${icm_summary_fact.cust_line_seq_id}=${churn_sum_fact.cust_line_seq_id} and ${icm_summary_fact.call_answer_dt_date}= date(${loyalty_with_churn.date});;
  }
}
